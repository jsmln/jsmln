<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:0a1f0f,50:166534,100:22c55e&height=220&section=header&text=Jasmin%20%20Esperida&fontSize=46&fontColor=ffffff&fontAlign=50&fontAlignY=35&desc=Full-Stack%20Developer%20%20%C2%B7%20%20LLM%20Integration%20%20%C2%B7%20%20Data%20Analytics&descColor=bbf7d0&descSize=17&descAlign=50&descAlignY=58&animation=fadeIn" />

<br/>

<a href="https://linkedin.com/in/jasmin-esperida">
  <img src="https://readme-typing-svg.demolab.com/?lines=Building+ATLAS+%E2%80%94+LLM-driven+EdTech+for+PH+HEIs;Full-Stack+Dev+%7C+React+%C2%B7+Django+%C2%B7+PostgreSQL;Turning+data+into+decisions,+one+dashboard+at+a+time;Badminton+player+%F0%9F%8F%B8+%C2%B7+%E2%98%95&font=Fira%20Code&center=true&width=600&height=45&duration=3200&pause=900&color=4ADE80&vCenter=true&size=20" />
</a>

<br/><br/>

```
  Final-year BSIT student · Major in Business Analytics
  Batangas State University TNEU – Lipa Campus
  She/Her  ·  Lipa City, Batangas, Philippines
```

<br/>

[![LinkedIn](https://img.shields.io/badge/LinkedIn-jasmin--esperida-166534?style=for-the-badge&logo=linkedin&logoColor=white&labelColor=0a1f0f)](https://linkedin.com/in/jasmin-esperida)
[![Email](https://img.shields.io/badge/Email-jasmin.esperida%40gmail.com-166534?style=for-the-badge&logo=gmail&logoColor=white&labelColor=0a1f0f)](mailto:jasmin.esperida@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-jsmln-166534?style=for-the-badge&logo=github&logoColor=white&labelColor=0a1f0f)](https://github.com/jsmln)

</div>

<br/>

<img src="https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/rainbow.gif" width="100%" height="4px"/>

<br/>

## 👩‍💻 &nbsp; About Me

<table width="100%">
<tr><td>

```python
class Jasmin:
    name        = "Jasmin B. Esperida"
    role        = "Full-Stack Developer  |  LLM Integration Lead  |  Data Analyst"
    university  = "Batangas State University TNEU – Lipa Campus"
    major       = "BS Information Technology, Major in Business Analytics"
    pronouns    = "she / her"

    currently_working_on = [
        "LLM-integrated web platforms with RAG + CoT architecture",
        "Prescriptive analytics pipelines for real-world institutional problems",
        "Multi-tenant PaaS systems with Django REST + React.js"
        "Mobile Application Development"
    ]

    interests = [
        "AI & LLM Application Development",
        "Full-Stack Web Development",
        "Data Analytics & Visualization",
        "EdTech & Social Impact Systems"
    ]

    fun_facts = [
        "Plays Badminton🏸",
        "Sleep is my love language 😴",
        "Swimming is my safe space",
        "Art is my Life --Art itself, and a person--my Isabel<3",
        
    ]

    reach_me_at = "esperidajasmin06@gmail.com"
```

</td></tr>
</table>

<br/>

<img src="https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/rainbow.gif" width="100%" height="4px"/>

<br/>

## 🛠️ &nbsp; Tech Stack

<div align="center">

**Languages**

<img src="https://skillicons.dev/icons?i=python,java,php,javascript,dart,r,cpp&theme=dark" />

<br/><br/>

**Frameworks & Libraries**

<img src="https://skillicons.dev/icons?i=react,django,flutter,tailwind,materialui&theme=dark" />

<br/><br/>

**Database**

<img src="https://skillicons.dev/icons?i=postgres,mysql&theme=dark" />

![pgvector](https://img.shields.io/badge/pgvector-16a34a?style=flat-square&logoColor=white) ![pgcrypto](https://img.shields.io/badge/pgcrypto-166534?style=flat-square&logoColor=white)

<br/><br/>

**AI & LLM Integration**

![RAG](https://img.shields.io/badge/Retrieval--Augmented_Generation-166534?style=flat-square&logoColor=white)
![CoT](https://img.shields.io/badge/Chain--of--Thought_Prompting-14532d?style=flat-square&logoColor=white)
![Prompt Engineering](https://img.shields.io/badge/Prompt_Engineering-16a34a?style=flat-square&logoColor=white)
![Claude API](https://img.shields.io/badge/Anthropic_Claude_API-166534?style=flat-square&logoColor=white)
![OpenRouter](https://img.shields.io/badge/OpenRouter-14532d?style=flat-square&logoColor=white)

<br/><br/>

**Data & Analytics**

<img src="https://skillicons.dev/icons?i=powerbi&theme=dark" />

![Jupyter](https://img.shields.io/badge/Jupyter-14532d?style=flat-square&logo=jupyter&logoColor=white) ![Excel](https://img.shields.io/badge/Microsoft_Excel-16a34a?style=flat-square&logo=microsoftexcel&logoColor=white) ![Fabric](https://img.shields.io/badge/Microsoft_Fabric-166534?style=flat-square&logoColor=white) ![Colab](https://img.shields.io/badge/Google_Colab-14532d?style=flat-square&logo=googlecolab&logoColor=white)

<br/><br/>

**Backend, Cloud & DevOps**

<img src="https://skillicons.dev/icons?i=redis,nginx,docker,githubactions&theme=dark" />

![Celery](https://img.shields.io/badge/Celery-166534?style=flat-square&logo=celery&logoColor=white) ![Gunicorn](https://img.shields.io/badge/Gunicorn-166534?style=flat-square&logo=gunicorn&logoColor=white) ![Java RMI](https://img.shields.io/badge/Java_RMI-14532d?style=flat-square&logoColor=white) ![DigitalOcean](https://img.shields.io/badge/DigitalOcean-16a34a?style=flat-square&logo=digitalocean&logoColor=white) ![Cloudflare](https://img.shields.io/badge/Cloudflare-166534?style=flat-square&logo=cloudflare&logoColor=white)

</div>

<br/>

<img src="https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/rainbow.gif" width="100%" height="4px"/>

<br/>

## 📂 &nbsp; Projects

<table width="100%">
<tr>
<td width="50%" valign="top">

### 🏛️ ATLAS
**LLM-Driven Instructional Management Platform**
`Apr 2026`

Multi-tenant PaaS automating OBE compliance workflows for Philippine HEI faculty — reducing syllabus prep from 7+ hours to under 20 minutes. Integrates RAG + CoT, Google Sheets API, and prescriptive analytics.

`Django` `React.js` `PostgreSQL` `pgvector` `Redis` `Celery` `Anthropic API`

🏅 *Best in System Requirements and Definition — ASIA 2026*

</td>
<td width="50%" valign="top">

### 🤝 Kooptimizer
**Cooperative Data Management System**
`Oct – Dec 2025`

Workflow optimization system for Lipa City Cooperatives Office. Reduced operational bottlenecks by **35%** and increased task completion accuracy by **25%**.

`PHP` `MySQL` `JavaScript`

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 🌱 ReGenEarth
**Eco-Solution Community Platform**
`Apr – May 2025`

Community-driven platform for eco-project collaboration and sustainable practices. API-integrated content discovery reducing manual submission time by **~30%**.

`PHP` `REST API` `MySQL`

</td>
<td width="50%" valign="top">

### 🏥 Hospital Patient Management System
**Web & Desktop Application**
`Oct – Dec 2024`

Full-stack hospital system for patient intake, discharge, treatment, and billing. Reduced manual documentation by **40%** across medical staff workflows.

`Java` `Java Swing` `PHP` `MySQL`

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 📱 Flutter API Integration App
**Mobile Application**
`2025`

Cross-platform mobile app integrating JSONPlaceholder Users API with unit testing, Postman documentation, and Android deployment.

`Dart` `Flutter` `REST API` `Postman`

</td>
<td width="50%" valign="top">

### 🖥️ Java RMI HRMS
**Human Resource Management System**
`2025`

Client-server HRMS using Java RMI with Swing GUI, DTOs, remote interfaces, and payroll computation panel built in Eclipse.

`Java` `Java RMI` `Java Swing` `Eclipse`

</td>
</tr>
</table>

<br/>

<img src="https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/rainbow.gif" width="100%" height="4px"/>

<br/>

## 📊 &nbsp; GitHub Stats

<div align="center">

<img height="165" src="https://github-readme-stats.vercel.app/api?username=jsmln&show_icons=true&theme=transparent&title_color=4ade80&icon_color=22c55e&text_color=e5e7eb&border_color=166534&count_private=true&hide_border=false" />
&nbsp;&nbsp;
<img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=jsmln&layout=compact&theme=transparent&title_color=4ade80&text_color=e5e7eb&border_color=166534&langs_count=8&hide_border=false" />

<br/><br/>

<img src="https://github-readme-streak-stats.herokuapp.com?user=jsmln&theme=transparent&background=00000000&border=166534&ring=16a34a&fire=22c55e&currStreakLabel=4ade80&sideLabels=4ade80&dates=aaaaaa&hide_border=false" />

<br/><br/>

<img src="https://github-readme-activity-graph.vercel.app/graph?username=jsmln&theme=react-dark&hide_border=true&bg_color=00000000&color=4ade80&line=22c55e&point=bbf7d0&area=true&area_color=166534" width="100%" />

</div>

<br/>

<img src="https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/rainbow.gif" width="100%" height="4px"/>

<br/>

<img src="https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/rainbow.gif" width="100%" height="4px"/>

<br/>

## 🌐 &nbsp; Community & Involvement

| Role | Organization | Period |
|:---|:---|:---|
| **Chief Communications Officer** | Tech Innovators Society (TechIS), BatStateU TNEU | Aug 2025 – Present |
| **Cartoonist** | The BYTE: CICS Official Publication, BatStateU TNEU | A.Y. 2025 – 2026 |
| **Documentations Lead** | Tech Innovators Society (TechIS), BatStateU TNEU | A.Y. 2024 – 2025 |
| **Volunteer** | Batangas Information Technology Society (BITS) | Apr 2025 – 2026 |

<br/>

---

<div align="center">

*"Code. Learn. Build. Repeat."*

<br/>

![Visitor Count](https://komarev.com/ghpvc/?username=jsmln&color=166534&style=flat-square&label=Profile+Views)

<br/>

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:22c55e,50:166534,100:0a1f0f&height=120&section=footer" />

</div>
